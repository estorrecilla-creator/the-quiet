...
def generate_cover_images(prompts, out_dir, size=DEFAULT_SIZE, model=MODEL):
    """
    Genera una imagen por cada prompt de `prompts` y las guarda en `out_dir`
    como 01.png, 02.png, ... (el orden numérico es el orden en el que
    main.py las usará para el vídeo, cada una con su propio movimiento de
    cámara). Devuelve la lista de rutas guardadas, en orden.
    """
    ...
    for i, prompt in enumerate(prompts, start=1):
        print(f"-> Generando imagen {i}/{len(prompts)}...")
